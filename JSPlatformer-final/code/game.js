...
function Doom(pos, ch) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  this.wobble = Math.random() * Math.PI * 2;
   }

Doom.prototype.type = 'doom';
...
var wobbleSpeed = 8;
  var wobbleDist = 0.07;
 Doom.prototype.act = function(step) {
       this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist;
...
Player.prototype.act = function(step, level, keys, type) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches obstacle and the player hasn't won
  // Player loses
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  }
	else if (type == "doom" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  }
