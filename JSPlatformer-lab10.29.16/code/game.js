...
function Vandoom(pos, ch) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(0.6, 0.6);
  this.wobble = Math.random() * Math.PI * 2;
   }

Vandoom.prototype.type = 'vandoom';
  ...
  //checks for an obstacle at a certain position
  //size is player size
  Level.prototype.obstacleAt = function(pos, size) {
    ...
    //everything outside of the boundries is treated like a wall	
    ...
    // y is row that we are in x is position	
...
var wobbleSpeed = 8;
  var wobbleDist = 0.07;
  Vandoom.prototype.act = function(step) {
       this.wobble += step * wobbleSpeed;
    var wobblePos = Math.sin(this.wobble) * wobbleDist;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
  };
  ...
  Player.prototype.act = function(step, level, keys) {
    ...
    if (otherActor)
	//This is suppose to make the player shrink down when it dies
      level.playerTouched(otherActor.type, otherActor);

    if (level.status == 'dead') {
  ...
  Level.prototype.playerTouched = function(type, actor) {
	...
	//This is suppose to make the player shrink down when it dies
	  ...
	  else if (type == 'vandoom' && this.status == null) {
      this.status = 'dead';
      this.finishDelay = 1;
